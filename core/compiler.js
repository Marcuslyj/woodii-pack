...
import { toUnixPath, tryExtensions, getSourceCode } from './utils/index.js'
import fs from 'fs'
...
const generator = GENERATOR.default;
...
class Compiler {
  ...
  async run(callback) {
    ...
    // 编译入口文件
    await this.buildEntryModule(entry);
  ...
  async buildEntryModule(entry) {
    // 循环每一个入口的文件名和路径
    const entryKeys = Object.keys(entry);
    let entryName
    while (entryKeys.length) {
      entryName = entryKeys.shift();
      const entryPath = entry[entryName]
      // 单个入口编译完成后，返回相应的入口模块对象，并存储到entries中
      const entryObj = await this.buildModule(entryName, entryPath);
  ...
  // 匹配loader处理
  async handleLoader(modulePath) {
    const matchLoaders = [];
    // 1. 获取所有传入的loader规则
    const rules = this.options.module.rules;
    rules.forEach(async (loader) => {
      ...
      for (let i = matchLoaders.length - 1; i >= 0; i--) {
        ...
        // require引入对应loader
        const { default: loaderFn } = await import(matchLoaders[i]);
  ...
  handleWebpackCompiler(moduleName, modulePath) {
    ...
    module.dependencies.forEach(async (dependency) => {
      const depModule = await this.buildModule(moduleName, dependency);
